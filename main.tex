...
\date{2017\footnote{Ten dokument skompilowano \today{} z klasą {\tt SGGW-thesis} w wersji \SGGWthesisVersion. Różni się ona od opisanej tutaj, lecz różnice w~korzystaniu z~niej są małe -- proszę przeczytać plik {\tt readme}. Aktualną wersję klasy można pobrać z~repozytorium~\cite{klasa-github} lub ze strony~\cite{www-LC-klasaLaTeX}.\\
\indent Ta wersja pracy zawiera \hyperref[sec:dod:dodatek]{użyteczny Dodatek~}\ref{sec:dod:dodatek} oraz przypisy. Wyjaśniono w~nich różnice między wersją pierwotną z~2017 roku a~obecną.
...
% Pakiety i ustawienia dodane ze względu na Dodatek (\appendix).
% Trzeba je dołączyć w preambule, więc nie można tego zrobić w Dodatku.
% Po usunięciu Dodatku w Państwa pracy poniższą sekcję można usunąć,
% chyba, że korzystając ze wzorców w Dodatku, używacie Państwo niektórych pakietów i ustawień.
\SHOWREMARKStrue
...
\usepackage{comment}
\definecolor{SGGWclrCite}{rgb}{0.00,0.50,0.00}
\definecolor{SGGWclrURL}{rgb}{0.00,0.00,0.50}
\definecolor{SGGWclrLink}{rgb}{0.70,0.00,0.00}
\definecolor{SGGWclrFile}{rgb}{0.50,0.00,0.50} % dark magenta
% W Państwa pracach ustawienia kolorów w PDF należy usunąć, bo kolory nie są korzystne w druku.
\hypersetup{colorlinks=true, linkcolor=SGGWclrLink,citecolor=SGGWclrCite,urlcolor=SGGWclrURL,filecolor=SGGWclrFile}
...
\item{\verb|\promotor| -- dane promotora pracy}\footnote{W dopełniaczu, np. {\tt \tbs{}promotor\{dra hab. inż.\ Jana Abackiego, prof.\ SGGW\}}\usp{}.},
...
% W Państwa pracy należy usunąć tę linię, a w pliku main.tex usunąć odniesienia do etykiet w tym pliku.
% Etykiety te mają wewnątrz łańcuch ":dod:". W każdym razie i tak będą Państwo zastępować treść tej pracy własną treścią.
% Wtedy można skasować cały podkatalog "/Dodatek".
